...
class QueryObserver:
    ...
    def __init__(self, seed: int, context: frozenset, archive: dict, experience: dict,
                 absolute_time_gate: float, relative_time_gate: int, query_path: str, db_string: str, savedir: str,
                learning_rate, n_estimators, subsample, criterion, min_samples_split,
                 min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start):
        # the model part
        self.seed = seed
    ...
    def __str__(self) -> str:
        return "Gradient Boosting Observer (est: {}, d: {}) on Context: {} using {} Experiences and Timeout: {}" \
            .format(self.n_estimators, self.max_depth, self.context, len(self.experience.keys()), self.timeout)
    ...
    def update_timeout(self) -> None:
        ...
        return
#learning_rate, n_estimators, subsample, criterion, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start
    def train(self) -> None:
        ...
        new_model = GradientBoostingClassifier(learning_rate=self.learning_rate,
        ...
        with open(self.savedir + "depths.json", "a") as depths:
...
def train_context_model(context_queries, train_queries, context, context_models, query_object_dict, db_string, savedir, mm_dict,
    ...
    if len(label_uniques) == 1:
        ...
        observer = QueryObserver(seed, context, archive, experience, a_timeout, p_timeout, query_path, db_string, savedir,
                                 learning_rate, n_estimators, subsample, criterion, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start)
        t0 = time.time()
        observer.train()
...
def evaluate_workload(query_path, seed, archive, enc_dict, mm_dict, wc_dict, p_timeout, a_timeout, db_string, savedir,
                      skipped_dict, use_context, args_test_queries: list, query_object_dict: dict,
                      use_cqd: bool, learning_rate, n_estimators, subsample, criterion, min_samples_split,
                      min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start):
    # load queries
    ...
    for context in alive_it(context_queries):
        context_models, train_time = train_context_model(context_queries, train_queries, context, context_models,
                                                         query_object_dict, db_string, savedir, mm_dict, enc_dict, wc_dict,
                                                         skipped_dict, d_type_dict, archive, seed, a_timeout,
                                                         p_timeout, query_path, learning_rate, n_estimators, subsample,
                                                         criterion, min_samples_split, min_samples_leaf, min_weight_fraction_leaf,
                                                         max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start)
...
def main(split_path, savedir, learning_rate, n_estimators, subsample, criterion, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start):
    ...
    parser.add_argument("-db", "--database", default="stack", help="Database the given queries should run on. "
                                                                   ...
                                                                   "possible too.")
    parser.add_argument("-a", "--archive", default="",
    ...
    parser.add_argument("-sd", "--savedir", default="",
    ...
    if os.path.exists(args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "initial_predictions.json"):
        print("File already exists: " + args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "initial_predictions.json")
        return

    #learning_rate, n_estimators, subsample, criterion, min_samples_split, min_samples_leaf, min_weight_fraction_leaf, max_depth, min_impurity_decrease, max_features, max_leaf_nodes, warm_start
    args_learn_path = args.learn
    args_learning_rate = float(learning_rate) #l
    args_n_estimators = int(n_estimators) #e
    args_subsample = float(subsample) #s
    args_criterion = str(criterion) #c
    args_min_samples_split = int(min_samples_split) #mins
    args_min_samples_leaf = int(min_samples_leaf) #minl
    args_min_weight_fraction_leaf = float(min_weight_fraction_leaf)#minwl
    args_max_depth = int(max_depth) #d
    args_min_impurity_decrease = float(min_impurity_decrease) #mini
    args_max_features = max_features #mf
    args_max_leaf_nodes = max_leaf_nodes #ml
    args_warm_start = bool(warm_start) #ws
    a = "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start)
    print("Using absolute/percentage based timeout: {}s / {}%".format(args_a_timeout, args_p_timeout))
    print("split_path:" + str(split_path))
    #################################################################
    init_predictions, final_predictions, training_time, forward_pass_time, critical_queries \
        = evaluate_workload(query_path, args_seed, args_archive, args_label_encoders, args_mm_dict, args_wildcard_dict,
                            args_p_timeout, args_a_timeout, args_db, args_save_path, args_skipped_dict, args_use_context,
                            args_test_queries, args_query_objects, args_cqd, args_learning_rate, args_n_estimators, args_subsample, args_criterion, args_min_samples_split, args_min_samples_leaf, args_min_weight_fraction_leaf, args_max_depth, args_min_impurity_decrease, args_max_features, args_max_leaf_nodes, args_warm_start)
    u.save_json(init_predictions, args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "initial_predictions.json")
    u.save_pickle(training_time, args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "training_times.pkl")
    u.save_json(forward_pass_time, args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "forward_pass_times.json")
    predictions_path = args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "initial_predictions.json"
    if args_cqd:
        u.save_json(final_predictions, args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "final_predictions.json")
        u.save_pickle(critical_queries, args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "critical_queries.pkl")
        predictions_path = args_save_path + "l" + str(learning_rate) + "e" + str(n_estimators) + "s" + str(subsample) + "c" + str(criterion) + "mins" + str(min_samples_split) + "minl" + str(min_samples_leaf) + "minwl" + str(min_weight_fraction_leaf) + "d" + str(max_depth) + "mini" + str(min_impurity_decrease) + "mf" + str(max_features) + "ml" + str(max_leaf_nodes) + "ws" + str(warm_start) + "final_predictions.json"
